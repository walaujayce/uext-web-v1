  ...
  const handleSelectAllAlert = () => {
    setSelectedAlert((prev) => {
      ...
      if (prev.length !== data.length) {
        return data.map((item) => item.bed_id);
      } else {
        return [];
      }
    });
      ...
      <div className="container">
        <div className="top-bar">
          ...
          >
            ...
            {isOverlayVisible && (
              <AlertBatchSetting callback={handleModifyPatientAlert} patientIDs={selectedAlert} />
            )}
          </div>
        </div>
        <div className="al">
          {data.length === 0 ? <div>{t("Alert.NoResident")}</div> : (
            <>
              {/* checkbox table */}
                  ...
                  {data.map((item) => {
                    return (
                      <div
                        ...
                        onMouseEnter={() => setSelectedHoverIndex(item.bed_id)}
                ...
                <div className="grid-list">
                  {data.map((item) => {
                    return (
                      <div
                        key={item.bed_id}
                        className={`grid-row ${selectedHoverIndex === item.bed_id ? "hover" : ""}`}
                      ></div>
                    );
                  })}
                </div>

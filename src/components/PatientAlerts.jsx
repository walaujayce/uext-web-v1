...
function PatientAlerts() {
  ...
  const fetchAlertList = async (patientid) => {
    try {
      ...
      const filteredAlert = data.find((item) => item.patientid === patientid);
      setAlertList(filteredAlert); // Update state with filtered object
      handleAlertState(filteredAlert);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    switch (macaddress) {
      case "80C9553B5BA0":
        ...
        break;
      case "80C9553B543C":
        patientid = "PA241304003";
        break;
      default:
        console.error("Unknown macaddress:", macaddress);
    }
    fetchAlertList(patientid);
  }, [macaddress]); // Include macaddress as a dependency if it changes

  useEffect(() => {
    console.log("Updated alertList:", alertList);
  }, [alertList]);

  const handleAlertState = (alert) =>{
    setNotificationToggleState(alert.enablealert1);
    setAlertRepeatToggleState(alert.enablealert2);
    setExitBedRateToggleState(alert.enablealert3);
    setPositionToggleState(alert.enablealert4);
    setRespHeartBeatToggleState(alert.enablealert5);
  ...
  // Notification
  const [notificationToggleState, setNotificationToggleState] =  useSessionStorageState("notificationToggleState", false);
